...
class PosOrder(models.Model):
    ...
    def create_from_ui(self, orders, draft=False):
        """Convert POS prescription payload into the normal Pharmacy workflow after POS checkout."""
        result = super().create_from_ui(orders, draft=draft)
        result_by_ref = {}
        for item in result or []:
            if isinstance(item, dict):
                ref = item.get('pos_reference') or item.get('name')
                if ref:
                    result_by_ref[ref] = item.get('id')

        for ui_order in orders:
            data = ui_order.get('data', {}) if isinstance(ui_order, dict) else {}
            payload = data.get('pharmacy_prescription') or data.get('pharmacyPrescription')
            if not payload:
                continue
            pos_ref = data.get('name') or data.get('pos_reference')
            order_id = result_by_ref.get(pos_ref)
            pos_order = self.browse(order_id).exists() if order_id else self.search([('pos_reference', '=', pos_ref)], limit=1)
            if pos_order and not pos_order.pharmacy_prescription_id:
                try:
                    pos_order._pharmacy_create_prescription_from_order(payload)
                except Exception:
                    _logger.exception('Failed to create pharmacy prescription from POS order %s', pos_order.display_name)
                    pos_order.message_post(body=_('Pharmacy prescription creation failed from POS payload. Please create/link manually.'))
        return result
    ...
    def _pharmacy_create_prescription_from_order(self, payload=None):
        ...
        file_data = payload.get('attachment') or payload.get('file_data') or payload.get('fileData') or self.pharmacy_prescription_attachment
        file_name = payload.get('attachment_name') or payload.get('file_name') or payload.get('fileName') or self.pharmacy_prescription_attachment_name or _('Prescription Upload')
        mime_type = payload.get('mimetype') or payload.get('mimeType') or False
        if file_data:
        ...
        self.write({
            ...
            'pharmacy_prescription_payload': json.dumps(payload),
            'pharmacy_prescription_notes': payload.get('notes') or payload.get('diagnosis_notes') or payload.get('diagnosisNotes') or self.pharmacy_prescription_notes or False,
            'pharmacy_prescription_attachment': file_data if file_data else self.pharmacy_prescription_attachment,
            'pharmacy_prescription_attachment_name': file_name if file_data else self.pharmacy_prescription_attachment_name,
        })
