...
/*
 * SPXCORP Pharmacy POS Actions Menu Button - V3.1.5
 * Places Prescription inside the POS three-dot Actions modal.
 * This avoids floating button placement issues and keeps the POS screen clean.
 */
(function () {
    "use strict";
    ...
    function openPrescriptionModal() {
        ...
        overlay.style.cssText = "position:fixed;inset:0;z-index:1000000;background:rgba(0,0,0,.45);display:flex;align-items:center;justify-content:center;font-family:Arial,sans-serif;";
        overlay.innerHTML = `
            <div style="width:760px;max-width:94vw;max-height:92vh;overflow:auto;background:#fff;border-radius:14px;box-shadow:0 20px 50px rgba(0,0,0,.28);">
                <div style="padding:16px 20px;border-bottom:1px solid #e5e7eb;display:flex;justify-content:space-between;align-items:center;background:#0B2B45;color:white;border-radius:14px 14px 0 0;">
                    ...
                    <button data-close="1" style="border:0;background:transparent;color:white;font-size:28px;cursor:pointer;">×</button>
                </div>
                <div style="padding:18px 20px;text-align:left;">
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                        ...
                        <div style="grid-column:1 / span 2;font-size:13px;color:#555;">${existing.fileName ? "Saved file: " + existing.fileName : "Upload prescription scan/photo if available."}</div>
                    </div>
                    <div style="margin-top:12px;padding:10px;background:#eef7e8;border:1px solid #b8e0a8;border-radius:8px;color:#276419;font-size:13px;">
                        Details are saved to the current POS order session and will be used for pharmacy prescription linkage.
                    </div>
                </div>
    ...
    function makeActionTile() {
        ...
        tile.addEventListener("click", () => {
            const closeBtn = document.querySelector('.modal button[aria-label="Close"], .modal .btn-close');
            openPrescriptionModal();
        });
        return tile;
    }

    function findActionsModalGrid() {
        const modals = Array.from(document.querySelectorAll("div"));
        const actionModal = modals.find((el) => {
            const text = (el.textContent || "");
            return text.includes("Actions") && text.includes("Refund") && text.includes("Cancel Order");
        });
        if (!actionModal) return null;

        const possibleGrids = Array.from(actionModal.querySelectorAll("div")).filter((el) => {
            const style = window.getComputedStyle(el);
            const text = (el.textContent || "");
            return text.includes("Refund") && text.includes("Cancel Order") &&
                   (style.display === "grid" || el.children.length >= 4);
        });

        return possibleGrids[0] || actionModal;
    }

    function injectIntoActionsModal() {
        if (document.getElementById("spx-pharmacy-action-tile")) return;

        const grid = findActionsModalGrid();
        if (!grid) return;

        const tile = makeActionTile();

        // Make it blend into the Actions popup as the next card.
        try {
    ...
    function removeOldFloatingButtons() {
        const old = document.getElementById("spx-pharmacy-pos-button");
        if (old) old.remove();
    }
    ...
    function start() {
        ...
        setInterval(() => {
            removeOldFloatingButtons();
            injectIntoActionsModal();
        }, 700);
        console.log("SPXCORP Pharmacy POS Actions menu prescription button loaded.");
    }
