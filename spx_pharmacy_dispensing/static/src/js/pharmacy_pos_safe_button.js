...
(function () {
    ...
    function injectButton() {
        ...
        // Preferred placement: the lower action row beside Customer / Note / Upload / Options.
        const candidates = Array.from(document.querySelectorAll("div, section, footer"));
        anchor = candidates.find((el) => {
            const text = (el.textContent || "").trim();
            return text.includes("Customer") && text.includes("Note") && !text.includes("Payment");
        });

        // Fallbacks if Odoo changes the DOM.
        anchor = anchor ||
        ...
        try {
            ...
            anchor.style.alignItems = anchor.style.alignItems || "center";
        } catch (e) {}

        const btn = document.createElement("button");
        btn.id = "spx-pharmacy-pos-button";
        btn.type = "button";
        btn.style.cssText = "padding:16px 24px;border:0;border-radius:8px;background:#0B2B45;color:white;font-weight:bold;box-shadow:0 2px 8px rgba(0,0,0,.16);cursor:pointer;margin-left:10px;min-width:170px;height:70px;font-size:18px;";
