...
class PharmacyAlertOverrideWizard(models.TransientModel):
    ...
    def default_get(self, fields_list):
        ...
        if not res.get('alert_id') and self.env.context.get('active_model') == 'pharmacy.safety.alert' and self.env.context.get('active_id'):
            res['alert_id'] = self.env.context.get('active_id')
        return res
